...
class AdsValidationV1(web.View):
	async def get(self):
		query = build_query('select', 'ads_validation', self.request.query)
		db = self.request.config_dict['db']
		try:
			rets = await db.execute(query, fetch=True)
			for i in range(len(rets)):
	...
	async def post(self):
		...
		query = build_query('insert', 'ads_validation', data)
		db = self.request.config_dict['db']
		try:
			await db.execute(query)
		except:
	...
	async def put(self):
		...
		query = build_query('update', 'ads_validation', data)
		db = self.request.config_dict['db']
		try:
			await db.execute(query)
		except:
	...
	async def delete(self):
		query = build_query('delete', 'ads_validation', self.request.query)
		db = self.request.config_dict['db']
		try:
			await db.execute(query)
		except:
